<template>
  <v-container>
    ...
    ></v-text-field>
    <br>
    <NuevoContacto :dialog = "dialog" @update:dialog = "handleUpdateDialog" :itemProp = "itemSelected" @update:contactos = "cargarDatos"/>
    <v-card>
      <v-card-title>
        <div class = "titulo">
          {{ titulo }}
...
export default {
  components: {
    ...
    ...mapActions({
      eliminarContacto: 'eliminarContacto', fetchContactos: 'fetchContactos'
    }), handleUpdateDialog(value) {
      this.dialog = value; // Actualiza la propiedad dialog cuando el hijo emite el evento update:dialog
    }, nuevoItem() {
      // Restablece el objeto itemSelected para un nuevo usuario
      this.itemSelected = {
        id: 0,
        tipo_contacto: '',
        nombre_completo: '',
      ...
      this.eliminarContacto(item.id)
          .then(() => { //Manejamos los mensajes en la store
            this.cerrarDialogo();
            this.cargarDatos();
            this.$emit('delete:contacto');
          })
          .catch(() => {
          });
    }, sortBy(column) {
      ...
      try {
        ...
        this.contactos = this.$store.getters.todosLosContactos; // Actualiza la propiedad contactos con el resultado del getter
      } catch (error) {
