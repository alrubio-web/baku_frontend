<template>
  <v-container>
    ...
    ></v-text-field>
    <br>
    <NuevoContrato :dialog = "dialog" @update:dialog = "handleUpdateDialog" :itemProp = "itemSelected" @update:contratos =
        "cargarDatos"/>
    <EliminarContratoCompleto :dialog = "dialog_eliminar" @update:dialog = "handleUpdateDialogEliminar" @delete:contrato =
        "cargarDatos"/>
    <v-card>
      <v-card-title>
        <div class = "titulo">
        ...
        <v-spacer></v-spacer>
        <div>
          <v-btn v-if="esAdmin" color="primary" dark class="mb-2 mr-2" @click="nuevoItem">
            Nuevo
          </v-btn>
          <v-btn v-if="esAdmin" dark class="mb-2" @click="eliminarCompleto">
            ¡Eliminar Contrato!<v-icon>mdi-alert</v-icon>
          </v-btn>
        </div>
...
import EliminarContratoCompleto from "@/components/contratos/EliminarContratoCompleto.vue";

export default {
  components: {
    EliminarContratoCompleto,
    NuevoContrato
  }, props: {
    ...
    ...mapActions({
      fetchVistaContratos: 'fetchVistaContratos', eliminarContrato: 'eliminarContrato'
    }), formatCurrency(value) {
      return formatCurrency(value);
    }, displayValue(item, header) {
      // Maneja el formateo de la divisa en la tabla
      if (['renta', 'deduccion_fiscal', 'fianza', 'saldo'].includes(header.value)) {
        return this.formatCurrency(item[header.value]);
      }
      // Maneja los campos booleanos
      if (header.value === 'estado') {
        return item[header.value] ? 'Activo' : 'Anulado';
      }
      if (header.value === 'sujeto_a_IRPF') {
        return item[header.value] ? 'Sí' : 'No';
      }
      if (header.value === 'estado') {
        return item[header.value] ? 'Activo' : 'Anulado';
      }
    ...
    }, handleUpdateDialogEliminar(value) {
      this.dialog_eliminar = value; // Actualiza la propiedad dialog cuando el hijo emite el evento update:dialog
    }, nuevoItem() {
