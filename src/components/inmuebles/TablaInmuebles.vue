<template>
  <v-container>
    ...
    ></v-text-field>
    <br>
    <NuevoInmueble :dialog = "dialog" @update:dialog = "handleUpdateDialog" :itemProp = "itemSelected" @update:inmuebles = "cargarDatos"/>
    <v-card>
      <v-card-title>
        <div class = "titulo">
          {{ titulo }}
          ...
          <tbody>
            <tr v-for = "item in filteredItems" :key = "item.id">
              <td v-for="header in cabecera" :key="header.value">
                <template v-if="aplicaTooltip(item, header)">
                  <v-tooltip :text="item[header.value]">
                    <template v-slot:activator="{ props }">
                      <div class="truncate-text" v-bind="props">
...
import NuevoInmueble from './NuevoInmueble.vue';
...
export default {
  components: {
    ...
    ...mapActions({
      eliminarInmueble: 'eliminarInmueble', fetchInmuebles: 'fetchInmuebles',
    }), formatCurrency(value) {
      return formatCurrency(value);
    }, displayValue(item, header) {
      // Maneja el formateo de la divisa en la tabla
      if (['precio_calculo_amoritzacion', 'precio_escriturado_transmision', 'valor_catastral_suelo',
        'valor_catastral', 'total_amortizaciones', 'valor_catastral_construccion'].includes(header.value)) {
    ...
    }, aplicaTooltip(item, header) {
      const value = item[header.value];
      return value && value.length > 30; // aplica el tooltip para textos mayores a 30 caracteres
    }, handleUpdateDialog(value) {
      this.dialog = value; // Actualiza la propiedad dialog cuando el hijo emite el evento update:dialog
    }, nuevoItem() {
      // Restablece el objeto itemSelected para un nuevo inmueble
      this.itemSelected = {
        id: 0,
        nombre: '',
        tipo: '',
      ...
      // Establece itemSelected al usuario a editar y abre el diálogo
      this.itemSelected = {...item};
      this.dialog = true;
    }, eliminarItem(item) {
      this.eliminarInmueble(item.id)
          .then(() => { //Manejamos los mensajes en la store
            this.cerrarDialogo();
            this.cargarDatos();
            this.$emit('delete:inmueble');
          })
          .catch(() => {
          });
    }, sortBy(column) {
      ...
      try {
        ...
        this.inmuebles = this.$store.getters.todosLosInmuebles; // Actualiza la propiedad inmuebles con el resultado del getter
      } catch (error) {
